...
"""Script to remove files in ~/.local/share/mediaselect-fr/logs directory"""

print(
    "Le script clean_var_tmp.py permet de supprimer les fichiers de "
    "logs les plus anciens présents dans le dossier "
    "~/.local/share/mediaselect-fr/logs de votre ordinateur."
)

cmd = "du -h ~/.local/share/mediaselect-fr/logs/ 2> /dev/null | tail -n 1"
...
cmd = "ls -p1t ~/.local/share/mediaselect-fr/logs | grep -v / | wc -l"
...
print(
    "La taille du dossier ~/.local/share/mediaselect-fr/logs est de " + size_tmp + " et il "
...
if answer.lower() == "oui":
    ...
    cmd = "ls -p1t ~/.local/share/mediaselect-fr/logs | grep -v / | tail -n {files_number}".format(
        files_number=files_number
    )
    # cmd = "ls -1t ~/.local/share/mediaselect-fr/logs | tail -n {files_number}".format(
    # files_number=files_number)
    ls = subprocess.Popen(
        cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True
    )
    ...
    if delete.lower() == "oui":
        cms = (
            "ls -p1t ~/.local/share/mediaselect-fr/logs | grep -v / | tail -n "
            + files_number
            + " | xargs -I {} rm ~/.local/share/mediaselect-fr/logs/{}"
        )
        ...
        print(
            "La taille du dossier ~/.local/share/mediaselect-fr/logs est "

...
logging.basicConfig(
    filename=os.path.expanduser(
        "~/.local/share/mediaselect-fr/logs/record_{title}_{save}.log".format(
    ...
    if int(proc_count) < 3 or (
        ...
        if args.recorder == "ffmpeg":
            if args.provider == "freeboxtv":
                cmd = (
                    ...
                    "_{record_position}_{save}.ts >> ~/.local/share/mediaselect-fr"
                ...
                cmd = (
                    ...
                    "{provider}_{record_position}_{save}.ts >> ~/.local/share/mediaselect-fr"
        ...
        if args.recorder == "streamlink":
            cmd = (
                ...
                "_{save}.ts -f {m3u8_link} best >> ~/.local/share/mediaselect-fr/logs/infos"
            ...
            cmd = (
                ...
                ">> ~/.local/share/mediaselect-fr/logs/infos_{title}_{provider}"
            ...
            cmd = (
                ...
                "/home/$USER/videos_select/{title}-save/{title}_{provider}"
                "_{record_position}_{save}.ts >> ~/.local/share/mediaselect-fr/logs/infos_{title}_{provider}"
                "_{record_position}_{save}.log 2>&1".format(
                    m3u8_link=args.m3u8_link,
                    title=args.title,
                    provider=args.provider,

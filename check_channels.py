...
base_config_dir = Path("/home") / user / ".config" / "mediaselect-fr" / "iptv_providers"
...
if junk_file.exists():
    try:
        rel_after_config = junk_file.resolve().as_posix().split(".config/mediaselect-fr/", 1)[1]
    ...
    if " = " in line:
        ...
        if split[1] != "":
            ...
            if provider_recorder == 1:
                ...
                cmd = [
                    str(Path.home() / ".local/share/mediaselect-fr/.venv/bin/streamlink"),

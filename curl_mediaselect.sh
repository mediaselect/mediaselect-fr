...
PYTHON="$HOME/.local/share/mediaselect-fr/.venv/bin/python"

CRYPTED_CREDENTIALS=$($PYTHON -c "import sys; sys.path.insert(0, '$HOME/.config/mediaselect-fr'); import config; print(config.CRYPTED_CREDENTIALS)")

LOG_FILE="$HOME/.local/share/mediaselect-fr/logs/cron_curl.log"
OUTPUT_FILE="$HOME/.local/share/mediaselect-fr/info_progs.json"
API_URL="https://www.media-select.fr/api/v1/prog"

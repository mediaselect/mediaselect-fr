...
create_dir_with_permissions(os.path.expanduser("~/.local/share/mediaselect-fr"), permission_mode)
create_dir_with_permissions(os.path.expanduser("~/.local/share/mediaselect-fr/logs"), permission_mode)
...
create_dir_with_permissions(os.path.expanduser("~/.config/mediaselect-fr/iptv_providers"), permission_mode)
...
try:
    response = requests.head("https://media-select.fr", timeout=timeout)
    response.raise_for_status()
except ConnectTimeout:
    print(f"Connection to MEDIA-select.fr timed out after {timeout} seconds")
    exit(1)
except ConnectionError:
    print("Failed to connect to MEDIA-select.fr")
    exit(1)
...
while crypted.lower() not in answers:
    crypted = input(
        ...
        "l'application web MEDIA-select.fr? Si vous répondez oui, "
...
config_path = os.path.join("/home", user, ".config/mediaselect-fr/config.py")
template_path = os.path.join("/home", user, "mediaselect-fr/config_template.py")
    ...
    if hdmi_screen == "non":
        ...
        if not gpg_keys:
            print(
                ...
                "la clé GPG sécurisé: https://media-select.fr/advice-gpg puis relancez le programme d'installation."
            )
            exit(1)
        else:
...
if hdmi_screen == "non":
    sleep(1)
    print(
        "Veuillez saisir l'email de votre compte à MEDIA-select.fr. L'email "
    ...
    try:
        insert_email = subprocess.run(["pass", "insert", "media-select/email"], check=False)
    except FileNotFoundError:
        print("Le gestionnaire 'pass' n'est pas installé ou non trouvé dans PATH.")
        exit(1)
    ...
    sleep(1)
    print(
        "Veuillez saisir le mot de passe de votre compte à MEDIA-select.fr. "
    ...
    try:
        insert_password = subprocess.run(["pass", "insert", "media-select/password"], check=False)
    except FileNotFoundError:
        print("Le gestionnaire 'pass' n'est pas installé ou non trouvé dans PATH.")
        exit(1)
else:
...
while http_status != 200:
    ...
    if hdmi_screen == "non":
        try:
            username_mediaselect = run_pass_get("media-select/email")
            password_mediaselect = run_pass_get("media-select/password")
            ...
            if try_again != "oui":
                ...
                username_mediaselect = input("Veuillez saisir de nouveau votre identifiant (email): ").strip()
                password_mediaselect = getpass.getpass("Veuillez saisir de nouveau votre mot de passe: ")

    timeout = 4

    try:
        response = requests.head(
            "https://www.media-select.fr/api/v1/prog",
    ...
    if http_status != 200:
        ...
        if try_again.lower() == "oui":
            if hdmi_screen == "oui" or hdmi_screen == "no_se":
                ...
                try:
                    subprocess.run(["pass", "insert", "media-select/email"], check=False)
                except FileNotFoundError:
                    print("Le gestionnaire 'pass' n'est pas installé ou non trouvé dans PATH.")
                    exit(1)
                sleep(1)
                ...
                try:
                    subprocess.run(["pass", "insert", "media-select/password"], check=False)
                except FileNotFoundError:
                    print("Le gestionnaire 'pass' n'est pas installé ou non trouvé dans PATH.")
                    exit(1)
        else:
...
username_mediaselect_safe = sanitize_token(username_mediaselect)
password_mediaselect_safe = sanitize_token(password_mediaselect)

try:
    position = None
    for idx, line in enumerate(lines):
        if line.strip() == "machine www.media-select.fr":
            position = idx
            break
    if position is not None:
        # Replace next lines or append as needed
        if position + 1 < len(lines):
            lines[position + 1] = f"  login {username_mediaselect_safe}"
        else:
            lines.insert(position + 1, f"  login {username_mediaselect_safe}")
        if crypted.lower() == "non":
            pwd_line = f"  password {password_mediaselect_safe}"
        else:
        ...
        lines.append(f"  login {username_mediaselect_safe}")
        if crypted.lower() == "non":
            lines.append(f"  password {password_mediaselect_safe}")
        else:
...
if hdmi_screen == "oui" and crypted.lower() != "non":
    ...
    try:
        keyring.set_password("media-select", "username", username_mediaselect_safe)
        keyring.set_password("media-select", "password", password_mediaselect_safe)
    except Exception as e:
...
curl = (
    ...
    "/user/$(id -u)/bus /bin/bash $HOME/mediaselect-fr/curl_"
    "mediaselect.sh\n".format(
        minute=minute,
        heure=heure,
    )
...
if hdmi_screen == "oui" or hdmi_screen == "no_se":
    cron_lines = [curl if "mediaselect-fr/curl_mediaselect.sh" in cron else cron for cron in cron_lines]
else:
    cron_lines = [cron for cron in cron_lines if "mediaselect-fr/curl_mediaselect.sh" not in cron]
...
if (hdmi_screen == "oui" or hdmi_screen == "no_se") and "mediaselect-fr/curl_mediaselect.sh" not in cron_lines_join:

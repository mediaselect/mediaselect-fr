...
try:
    ...
    print(f"Failed to connect to MEDIA-select.fr")
...
with open("/home/" + user + "/.config/mediaselect-fr/config.py", "w") as conf:
    for param in params:
...
if hdmi_screen == "non":
    ...
    insert_email = subprocess.run(["pass", "insert", "media-select/email"])
    sleep(1)
    ...
    insert_password = subprocess.run(["pass", "insert", "media-select/password"])
else:
...
while http_status != 200:
    ...
    if hdmi_screen == "non":
        pass_email = subprocess.run(["pass", "media-select/email"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        username_mediaselect = pass_email.stdout.strip()
        pass_password = subprocess.run(["pass", "media-select/password"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        password_mediaselect = pass_password.stdout.strip()

    timeout = 4

    try:
        response = requests.head("https://www.media-select.fr/api/v1/prog", auth=(username_mediaselect, password_mediaselect), timeout=timeout)
    except ConnectTimeout:
        print(f"Connection to MEDIA-select.fr timed out after {timeout} seconds")
    except ConnectionError:
        print(f"Failed to connect to MEDIA-select.fr")
